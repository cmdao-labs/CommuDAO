...
const BBQAncientForrest = ({ config, navigate, callMode, setisLoading, txupdate, setTxupdate, setisError, setErrMsg, cmdsV2ABI, uplevelCMDSABI, fieldWoodBBQABI, partyABI, missionCMDBaseABI, missionWoodABI }) => {
    ...
    React.useEffect(() => {
        ...
        const thefetch = async () => {
            const nftGenesis = chain !== undefined && chain.id === 190 && address !== null ? await readContract(config, {
                address: CMDS,
                abi: cmdsV2ABI,
                functionName: 'mynft',
                args: [address],
            ...
            const data = chain !== undefined && chain.id === 190 && address !== null ? await readContracts(config, {
                contracts: [
                    {
                        address: CMDS,
                        abi: cmdsV2ABI,
            ...
            const woodBal = address !== null ? await readContract(config, {
                address: fieldWood,
                abi: fieldWoodBBQABI,
            ...
            const dataParty = chain !== undefined && chain.id === 190 && address !== null ? await readContracts(config, {
                contracts: [
                    {
            ...
            }) : [{result: 0}, {result: [0, 0, 0, 0, 0, 0]}, {result: [0, 0, 0, 0, 0, 0]}, {result: 0}, {result: 0}, {result: 0, status: 'yo'}, {result: 0, status: 'yo'}]
    ...
    const missionHarvestHandle = async (_nftid) => {
        ...
        try {
            ...
            if (Number(ethers.utils.formatEther(woodAllow)) < 1e12) {
                let { request } = await simulateContract(config, {
                    address: fieldWood,
                    abi: fieldWoodBBQABI,
                    functionName: 'approve',
                    args: [missionWood, ethers.constants.MaxUint256],
                })
                let h = await writeContract(config, request)
                await waitForTransactionReceipt(config, { hash: h })
            }
            ...
            {address !== null && chain !== undefined && chain.id !== 190 ?
                <div style={{zIndex: "999"}} className="centermodal">
                    <div className="wrapper">
                        ...
                        <div style={{marginTop: "20px", width: "90%", textAlign: "left", fontSize: "14px"}}>Please switch your network to CMDAO BBQ CHAIN.</div>
                ...
                <div style={{margin: "0", paddingTop: "30px", alignItems: "flex-start", justifyContent: 'space-between'}} className="collection">
                    <div style={{width: "95%", minHeight: "120px", height: "fit-content", margin: "10px", padding: "20px", fontSize: "10px", flexDirection: "row", justifyContent: "space-around", flexWrap: "wrap"}} className="nftCard">                
                        <div style={{height: "82%", display: "flex", flexDirection: "column", justifyContent: "space-around"}} className="bold">
                            ...
                            <>
                                {nft[0] !== null ?
                                    <>
                                        {nft.map((item, index) => (
                                            <>
                                                <div style={{justifyContent: "space-around", height: "500px", margin: "10px"}} className="nftCard" key={index}>
                                                ...
                                                <div className="nftCard" style={{position: "relative", justifyContent: "center", margin: "10px", height: "500px"}}>
