...
contract CMDAO_HOUSE_STAKING is ERC721Holder, ReentrancyGuard, Ownable {
    ...
    function stake(
        ...
        require(slot == 1, "Not Eligible NFT Type");
        require(CMCITY_SLOT1(cmcitySlot1).slotOwner(_house) == msg.sender, "No Permission");
        require(slotUsage[_pid][_house] < CMCITY_SLOT1(cmcitySlot1).slotLevel(_house), "Unavailable Slot");
    ...
    function unstake(
        ...
        if (_pending > 0) {
            IERC20(rewardToken).transfer(msg.sender, _pending);
        }
        stakednft.rewardDebt = (stakednft.power * pool.accRewardPerShare) / 1e12;
