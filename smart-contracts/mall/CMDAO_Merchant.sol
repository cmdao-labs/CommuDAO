// SPDX-License-Identifier: UNLICENSED

pragma solidity >=0.8.0;

import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import "@openzeppelin/contracts/token/ERC721/utils/ERC721Holder.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract CMDAO_Merchant is ERC721Holder, ReentrancyGuard {

    address public projectAdmin;
    modifier onlyProjectAdmin() {
        require(msg.sender == projectAdmin, "NP"); // NP : Not Permission to call
        _;
    }
    
    mapping(uint256=>address) public tokens;
    address public commudaoNfts;

    struct Sell {
        uint256 tokenIndex;
        uint256 price;
        uint256 sellId;
        uint256 endId;
    }
    mapping(uint256=>Sell) public sellList;
    
    constructor(address _commudaoNfts) {
        projectAdmin = msg.sender;
        commudaoNfts = _commudaoNfts;
    }

    function setProjectAdmin(address _addr) external onlyProjectAdmin {
        projectAdmin = _addr;
    }

    function setToken(uint256 _index, address _addr) external onlyProjectAdmin {
        tokens[_index] = _addr;
    }

    function withdrawToken(
        uint256 _tokenindex,
        uint256 _amount,
        address _to
    ) external onlyProjectAdmin {
        IERC20(tokens[_tokenindex]).transfer(_to, _amount);
    }

    function withdrawNFT(
        uint256 _nftId,
        address _to
    ) external onlyProjectAdmin {
        IERC721(commudaoNfts).transferFrom(address(this), _to, _nftId);
    }

    function setSell(
        uint256 _sellindex,
        uint256 _tokenIndex,
        uint256 _price,
        uint256 _sellId,
        uint256 _endId
    ) external onlyProjectAdmin {
        sellList[_sellindex].tokenIndex = _tokenIndex;
        sellList[_sellindex].price = _price;
        sellList[_sellindex].sellId = _sellId;
        sellList[_sellindex].endId = _endId;
    }

    function buy(uint256 _sellindex) external nonReentrant {
        IERC20(tokens[sellList[_sellindex].tokenIndex]).transferFrom(msg.sender, address(this), sellList[_sellindex].price);

        if (sellList[_sellindex].sellId <= sellList[_sellindex].endId) {
            IERC721(commudaoNfts).transferFrom(address(this), msg.sender, sellList[_sellindex].sellId);
            sellList[_sellindex].sellId += 100000;
        }
    }
   
}